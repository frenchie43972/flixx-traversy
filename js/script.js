const global = {
  ...
  api: {
    apiKey: 'faa95c551c75bdbcdf5143db2c813ec6',
...
async function displayPopularMovies() {
  ...
  results.forEach((movie) => {
    ...
    // If there is a movie.poster_path in the API, show the image
    // else so the no image, img
    div.innerHTML = `
...
async function displayPopularShows() {
  ...
  results.forEach((show) => {
    ...
    // If there is a show.poster_path in the API, show the image
    // else so the no image, img
    div.innerHTML = `
...
function displayPagination() {
  ...
  // Disable next button if on filastrst page
